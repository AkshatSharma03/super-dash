// Shared UI primitives across modes.
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "duration-150 border-none cursor-pointer",
        active
...
export function KPI({ label, value, sub, color = "#00AAFF", trend }: KPIProps) {
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 right-0 w-16 h-16 rounded-[0_0_0_60px] pointer-events-none"
        style={{ background: color + "08" }}
      />
      <p
        className={cn(
          "text-[10px] uppercase tracking-[0.8px] font-semibold",
      ...
      <p
        className="text-[22px] font-extrabold mb-0.5 tracking-[-0.5px]"
        style={{ color }}
      >
        {value}
      </p>
...
export function Card({ title, children }: CardProps) {
  return (
    <div className="bg-muted rounded-xl p-4 border border-border mb-4">
      <h3 className="text-[11px] text-muted-foreground font-bold uppercase tracking-[0.6px] mb-3.5">
        {title}
      </h3>
      {children}
    </div>
  ...
  return (
    ...
    >
      <div className="mb-3.5">
        ...
        <h3 className="text-[13px] text-foreground font-bold tracking-[-0.2px] mb-1">
          {title}
        </h3>
        {subtitle && (
...
export function Stat({
  ...
  return (
    <div className="text-center">
      <p className="text-[10px] text-muted-foreground uppercase tracking-[0.8px] mb-0.5">
        {label}
      </p>
...
export function MarkdownText({ text }: { text?: string }) {
  ...
  return (
    <Markdown
      ...
      components={{
        h2: ({ children }) => (
          <h4 className="text-base font-black text-memphis-black mt-4 mb-2">
            {children}
          </h4>
        ),
        h3: ({ children }) => (
          <h4 className="text-sm font-bold text-memphis-black mt-3.5 mb-1.5">
            {children}
          </h4>
        ),
        p: ({ children }) => (
          <p className="text-[14px] text-memphis-black/85 leading-[1.8] mb-3">
            {children}
          </p>
        ),
        ul: ({ children }) => (
        ...
        li: ({ children }) => (
          <li className="text-[14px] text-memphis-black/85 leading-[1.75]">
            {children}
          </li>
        ),
        strong: ({ children }) => (
...
export function ChartCard({ chart }: { chart: ChartConfig }) {
  ...
  const handleSVG = () => {
    ...
    downloadBlob(
      `${chart.title.replace(/[^a-z0-9]/gi, "_")}.svg`,
      serialized,
      "image/svg+xml",
    );
  };
  ...
  const handleCSV = () => {
    ...
    const rows = chart.data.map((row) =>
      headers.map((k) => row[k] as string | number | null),
    );
    downloadBlob(
      ...
      <div className="flex items-start gap-2 mb-1">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <h3 className="text-[13px] text-foreground font-bold">
              {chart.title}
            </h3>
            {src && (
              <a
                ...
                title={[
                  `Verified data · ${src.indicatorName}`,
                  `Retrieved ${new Date(src.retrievedAt).toLocaleDateString()}`,
                ].join(" · ")}
                className={cn(
                  "inline-flex items-center gap-1 text-[9px] font-bold px-1.5",
                  "py-0.5 rounded text-emerald-400 border",
                  "border-emerald-400/30 bg-emerald-500/10 no-underline",
                  "uppercase tracking-[0.5px] shrink-0",
      ...
      {sources.map((s, i) =>
        s.url ? (
          <a
            ...
            rel="noopener noreferrer"
            className={cn(
              "text-[11px] text-primary bg-card border border-border/70 rounded",
              "px-2 py-0.5 no-underline hover:border-primary/40",
              "transition-colors",
            )}
          >
            {s.title} ↗
          ...
          <span
            key={i}
            className={cn(
              "text-[11px] text-muted-foreground bg-card border border-border",
              "rounded px-2 py-0.5",
            )}
          >
            {s.title}
        ...
        <BarChart data={data} margin={M}>
          ...
          {series.map((s, i) => (
            <Bar
              ...
              fill={s.color ?? P[i]}
              stackId={s.stacked ? "a" : undefined}
              radius={!s.stacked ? [3, 3, 0, 0] : undefined}
            />
          ))}
        </BarChart>
      </ResponsiveContainer>
