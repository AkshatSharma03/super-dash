...
import type { CountryDataset, DataQualityCell, DataQualityStatus } from "../../types";
import { cn } from "@/lib/utils";

type QualityColorMap = Record<DataQualityStatus, { bg: string; text: string; label: string }>;

const QUALITY_COLORS: QualityColorMap = {
  complete:  { bg: "bg-emerald-200",   text: "text-emerald-800",  label: "Complete" },
  partial:   { bg: "bg-amber-100",    text: "text-amber-800",    label: "Partial" },
  estimated: { bg: "bg-orange-200",   text: "text-orange-800",   label: "Estimated" },
...
function deriveQuality(dataset: CountryDataset): DataQualityCell[] {
  ...
  const indicators: Array<{ key: string; label: string; getValue: (y: number) => { status: DataQualityStatus; display: string } }> = [
    {
      key: "gdp_bn",
      label: "GDP ($B)",
      getValue: (y) => {
    ...
    },
    {
      ...
      getValue: (y) => {
        const d = gdpMap.get(y);
        if (!d || d.gdp_growth == null) return { status: "missing", display: "—" };
    ...
    },
    {
      ...
      getValue: (y) => {
        const d = gdpMap.get(y);
        if (!d || d.gdp_per_capita == null) return { status: "missing", display: "—" };
    ...
    },
    {
      ...
      getValue: (y) => {
        ...
        if (typeof e.total !== "number") return { status: "estimated", display: "est." };
    ...
    },
    {
      ...
      getValue: (y) => {
        ...
        if (typeof im.total !== "number") return { status: "estimated", display: "est." };
    ...
    },
    {
      ...
      getValue: (y) => {
        ...
        if (filled < sectors.length) return { status: "partial", display: `${filled}/${sectors.length}` };
    ...
    },
    {
      ...
      getValue: (y) => {
        ...
        if (filled === 0) return { status: "missing", display: "—" };
        if (filled < partners.length) return { status: "partial", display: `${filled}/${partners.length}` };
...
export default function DataQualityHeatmap({ dataset }: Props) {
  ...
  const indicatorLabels = useMemo(() =>
    Object.fromEntries(qualityData.map(c => [c.indicator, c.indicatorLabel])),
    [qualityData]
  );
  ...
  const stats = useMemo(() => {
    ...
    const estimated = qualityData.filter(c => c.status === "estimated").length;
  ...
  const [tooltip, setTooltip] = useState<{ cell: DataQualityCell; x: number; y: number } | null>(null);

  const visibleYears = allYears.length > 20
  ...
  return (
    <div className="border-3 border-memphis-black bg-white shadow-hard-sm">
      <div className="px-4 py-3 border-b-2 border-memphis-black/10">
        <h3 className="text-xs font-black uppercase tracking-wider text-memphis-black/70">Data Coverage</h3>
        <p className="text-[10px] text-memphis-black/50 mt-0.5">
          {stats.complete}% complete · {stats.estimated}% estimated · {stats.partial}% partial · {stats.missing}% missing
        </p>
      </div>
      ...
      <div className="p-4 overflow-x-auto">
        <div className="min-w-fit">
          <div className="flex">
            ...
            {visibleYears.map(y => (
              <div key={y} className="w-8 text-center text-[9px] text-memphis-black/50 font-mono shrink-0">{y}</div>
            ))}
          </div>

          {indicators.map((ind) => (
            <div key={ind} className="flex items-center">
              <div className="w-28 shrink-0 text-[10px] text-memphis-black/70 font-medium truncate pr-2">
                {indicatorLabels[ind]}
              </div>
              {visibleYears.map(y => {
                ...
                return (
                  <div
                    key={`${y}:${ind}`}
                    className={cn(
                      "w-8 h-6 border border-black/5 flex items-center justify-center cursor-default transition-colors",
                      colors.bg
                    )}
                    onMouseEnter={(e) => {
                      if (cell) {
                  ...
                  >
                    <span className={cn("text-[8px] font-medium", colors.text)}>
                      {status === "complete" ? "●" : status === "missing" ? "—" : status === "estimated" ? "≈" : "◐"}
                    </span>
                  </div>
                );
              })}
      ...
      <div className="px-4 py-2 border-t-2 border-memphis-black/10 flex items-center gap-4 flex-wrap">
        {(Object.entries(QUALITY_COLORS) as [DataQualityStatus, typeof QUALITY_COLORS.complete][]).map(([status, colors]) => (
          <div key={status} className="flex items-center gap-1.5">
            <span className={cn("w-3 h-3 rounded-sm border border-black/10", colors.bg)} />
            <span className="text-[10px] text-memphis-black/60">{colors.label}</span>
          </div>
        ))}
      </div>
      ...
      {tooltip && (
        <div
          className="fixed z-50 bg-white border-2 border-memphis-black shadow-hard-sm px-3 py-2 text-[11px] pointer-events-none"
          style={{
            left: `${tooltip.x}px`,
            top: `${tooltip.y - 40}px`,
        ...
        >
          <p className="font-bold text-memphis-black">{tooltip.cell.indicatorLabel} · {tooltip.cell.year}</p>
          <p className={cn("font-medium", QUALITY_COLORS[tooltip.cell.status].text)}>
