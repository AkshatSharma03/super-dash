// Auth landing page for unauthenticated users.
...
const FEATURES: [ComponentType<{ className?: string }>, string, string][] = [
  [
    MessageSquare,
    "AI-powered charts",
    "Ask any economic question in plain language — get publication-ready " +
      "interactive charts in seconds",
  ],
  [
    Search,
    "Live web search",
    "Pull current data directly from World Bank, IMF, OECD, and national statistics offices",
  ],
  [
    Database,
    "Your own data",
    "Upload any CSV and get instant AI-powered charts and expert analysis",
  ],
  [
    LineChart,
    "Deep analytics",
    "OLS regression, K-Means clustering, HHI concentration, and anomaly detection on real data",
  ],
  [
    Globe2,
    "Global dashboards",
    "Instant dashboards for any country — US, China, EU, Japan and more — " +
      "15+ years of GDP, trade, and sector data",
  ],
];
...
const clerkAppearance = {
  ...
  elements: {
    ...
    socialButtonsBlockButton:
      "border-3 border-memphis-black rounded-none shadow-hard text-memphis-black font-black",
    socialButtonsBlockButtonText: "font-black",
    formFieldLabel:
      "text-[11px] uppercase tracking-wide font-black text-memphis-black",
    formFieldInput:
      "h-11 border-3 border-memphis-black/20 rounded-none text-memphis-black " +
      "placeholder:text-memphis-black/40 focus:border-memphis-pink focus:ring-0",
    formFieldInputShowPasswordButton: "text-memphis-black",
    formButtonPrimary:
      "h-10 rounded-none border-3 border-memphis-black bg-memphis-pink " +
      "text-white font-black uppercase tracking-wide shadow-hard " +
      "hover:bg-memphis-pink",
    ...
    dividerText:
      "text-[10px] uppercase tracking-wide font-black text-memphis-black/50",
...
export default function AuthPage({ onGuestAuth }: AuthPageProps) {
  ...
  const renderAuthForm = () => (
    <>
      ...
      <div className="flex bg-white border-3 border-memphis-black p-1 mb-5 shadow-hard">
        {(["login", "register"] as const).map((t) => (
          <button
            ...
            className={cn(
              "flex-1 py-2 text-xs font-black transition-snap border-3",
              "cursor-pointer uppercase tracking-wide",
              view === t
                ? "bg-memphis-pink text-white border-memphis-black shadow-hard-sm"
                : "bg-transparent text-memphis-black border-transparent",
            )}
          >
      ...
      <p className="text-center text-[10px] text-border mt-2">
        No email required · Chat history not saved
      </p>
    </>
  );
  ...
  return (
    <div
      className="bg-background min-h-[100dvh] text-foreground"
      style={{ fontFamily: "Inter,sans-serif" }}
    >
      {/* ── Top nav ── */}
      <nav
        className={cn(
          "px-3 sm:px-8 py-3 border-b-3 border-memphis-black flex",
          "items-center gap-2 sm:gap-3 bg-white",
        )}
      >
        <div
          className={cn(
            "w-8 h-8 border-3 border-memphis-black flex items-center",
            "justify-center text-base shadow-hard-sm",
          )}
        ...
        <span
          className={cn(
            "text-[13px] sm:text-[15px] font-black text-memphis-black tracking-tight",
            "uppercase",
          )}
        >
          EconChart
      ...
      <div className="flex min-h-[calc(100dvh-63px)] flex-col lg:flex-row">
        ...
        <div
          className={cn(
            "order-2 lg:order-1 flex-1 min-w-0 px-5 sm:px-8 lg:px-14",
            "py-6 sm:py-10 lg:py-[60px] flex flex-col justify-center",
          )}
        >
          <div className="max-w-[540px]">
            <div
              className={cn(
                "text-[10px] font-black uppercase tracking-[2px] mb-5",
                "bg-memphis-pink/10 border-2 border-memphis-pink inline-flex",
                "items-center gap-1.5 px-3.5 py-1.5 text-memphis-pink",
              )}
            >
              <span
            ...
            <h1
              className={cn(
                "text-[28px] sm:text-[32px] lg:text-[38px] font-black",
            ...
            <div className="flex flex-col gap-3.5">
              {visibleFeatures.map(([Icon, title, desc]) => (
                <div key={title} className="flex gap-3 items-start">
                  <div
                    className={cn(
                      "w-8 h-8 sm:w-9 sm:h-9 border-2 border-memphis-black",
                  ...
                  <div>
                    <div className="text-[13px] font-semibold text-memphis-black mb-0.5">
                      {title}
                    </div>
                    <div className="text-xs text-memphis-black/60 leading-[1.55]">
                      {desc}
                    </div>
                  </div>
                </div>
              ...
              {isMobile && FEATURES.length > visibleFeatures.length && (
                <p className="text-[11px] text-memphis-black/60 font-semibold">
                  +{FEATURES.length - visibleFeatures.length} more capabilities
                  after sign-in
                </p>
              )}
            </div>
        ...
        <div
          className={cn(
            "order-1 lg:order-2 w-full lg:w-[460px] shrink-0 px-5",
            "sm:px-8 lg:px-11 py-6 sm:py-10 lg:py-[60px]",
            "border-b-3 lg:border-b-0 lg:border-l-3 border-memphis-black",
            "flex flex-col justify-center bg-white",
          )}
        >
          <div className="w-full max-w-[460px] mx-auto">{renderAuthForm()}</div>
        </div>
      </div>
    </div>
  );
