...
export default function DeveloperPanel({ token, onClose }: Props) {
  const [keysResponse, setKeysResponse] =
    useState<DeveloperKeysResponse | null>(null);
      ...
      >
        ...
        <div className="p-5">
          <p className="text-xs text-memphis-black/70 mb-4">
            Use API keys to fetch country indicators and batch exports from the
            public API. Rate limit is tied to your subscription tier.
          </p>

          <div className="grid md:grid-cols-3 gap-3 mb-5">
            <div className="bg-memphis-offwhite border-3 border-memphis-black px-3 py-2.5">
              <p className="text-[11px] text-memphis-black/60">
                Plan monthly API limit
              </p>
              <p className="text-lg font-black">
                {planLimit === null ? "Unlimited" : `${planLimit} calls`}
              </p>
            </div>
            <div className="bg-memphis-offwhite border-3 border-memphis-black px-3 py-2.5">
          ...
          {error && (
            <div
              className={cn(
                "mb-4 p-2 border-3 border-red-400/80 bg-red-50 text-red-700 text-xs",
                "flex gap-1.5",
              )}
            >
          ...
          <form
            ...
            className="mb-5 border-3 border-memphis-black bg-memphis-offwhite p-3.5"
          >
            ...
            <div className="flex gap-2 flex-col sm:flex-row">
              <input
                ...
                className={cn(
                  "flex-1 border-3 border-memphis-black px-2 py-2 text-xs",
            ...
            {createError && (
              <p className="text-[11px] text-red-600 mt-2">{createError}</p>
            )}
          </form>
          ...
          {createdSecret && (
            <div className="mb-5 border-3 border-memphis-black bg-emerald-50 p-3">
              ...
              <p className="text-[11px] text-emerald-800 mb-2">
                This is your only chance to view this key. Save it now.
              </p>
              <div className="flex gap-2">
                <code
                  className={cn(
                    "flex-1 px-2 py-1.5 border border-emerald-300 bg-white text-xs",
                    "break-all",
                  )}
                >
                  {createdSecret}
          ...
          <div>
            <h3 className="text-xs font-black uppercase tracking-wide mb-2">
              API keys
            </h3>

            {loading ? (
              <div className="text-center py-6 text-memphis-black/50">
                Loading keys…
              </div>
            ) : !keys.length ? (
              <div
                className={cn(
                  "text-xs text-memphis-black/60 border-2 border-dashed",
                  "border-memphis-black/30 p-3",
                )}
              >
              ...
              <div className="border-3 border-memphis-black">
                {keys.map((keyRow) => {
                  const usageLimit =
                    keyRow.rateLimit === null ? "∞" : keyRow.rateLimit;
                  const usageLeft =
                    keyRow.callsRemaining === null
                      ? "∞"
                      : keyRow.callsRemaining;
                  ...
                  return (
                    <div
                      key={keyRow.id}
                      className="border-t border-memphis-black p-3 text-xs first:border-t-0"
                    >
                      <div className="flex gap-2 items-start justify-between">
                        <div className="min-w-0 flex-1">
                          <div className="font-black mb-0.5">{keyRow.name}</div>
                          <div className="text-[11px] text-memphis-black/70 mb-1.5">
                            {preview}
                          </div>
                          <div className="text-[11px] text-memphis-black/70">
                            ...
                            <span>
                              Last used {formatDateTime(keyRow.lastUsedAt)}
                            </span>
                          </div>
                        </div>
                        <Button
                          size="sm"
                          className={cn(
                            "shrink-0 border border-red-500 text-red-700",
                            "bg-white hover:bg-red-50",
                          )}
                          onClick={() => handleDelete(keyRow.id)}
                          disabled={deletingKeyId === keyRow.id}
                        >
                          {deletingKeyId === keyRow.id ? (
                            <Loader2 className="w-3.5 h-3.5 animate-spin" />
                          ) : (
          ...
          <div className="text-[10px] text-memphis-black/50 mt-4">
            ...
            <code>Authorization: Bearer &lt;key&gt;</code> and support{" "}
            <strong>json</strong> (default) and <strong>csv</strong>.
          </div>
        </div>
      </div>
    </div>,
