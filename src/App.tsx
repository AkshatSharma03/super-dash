...
import {
  ...
  PIE_EXPORTS_2024, PIE_IMPORTS_2024,
  KPI_SUMMARY, CHAT_SUGGESTIONS, SEARCH_SUGGESTIONS,
...
type Mode = "dashboard" | "chat" | "search" | "data" | "analytics";

// ─────────────────────────────────────────────────────────────────────────────
// CHART COLOR CONSTANTS
// ─────────────────────────────────────────────────────────────────────────────
const P = ["#00AAFF","#F59E0B","#10B981","#EF4444","#8B5CF6","#F97316","#06B6D4","#94a3b8"];
const C = { cn:"#EF4444", ru:"#F59E0B", eu:"#10B981", us:"#8B5CF6", tr:"#F97316", uk:"#06B6D4", other:"#64748b" } as const;
...
function Btn({ onClick, children, active, style = {}, disabled = false }: BtnProps) {
  return (
...
function KPI({ label, value, sub, color = "#00AAFF", trend }: KPIProps) {
...
// ─────────────────────────────────────────────────────────────────────────────
function DynChart({ chart }: { chart: ChartConfig }) {
    ...
    <ResponsiveContainer width="100%" height={h}>
      <PieChart>
        <Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%"
          outerRadius={95} label={({ name, value }: { name: string; value: number }) => `${name}: ${value}`} labelLine>
          {data.map((_entry, i) => <Cell key={i} fill={P[i % P.length]} />)}
        </Pie>
...
// ─────────────────────────────────────────────────────────────────────────────
async function askClaude(messages: Array<{ role: string; content: string }>): Promise<AIResponse> {
...
function DashboardMode({ yearRange, setYearRange: _setYearRange }: DashboardModeProps) {
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={PIE_EXPORTS_2024} dataKey="value" nameKey="name"
                  ...
                  {PIE_EXPORTS_2024.map((_entry, i) => <Cell key={i} fill={P[i % P.length]} />)}
                </Pie>
                <Tooltip {...TT} />
              </PieChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={PIE_IMPORTS_2024} dataKey="value" nameKey="name"
                  cx="50%" cy="50%" outerRadius={80}
                  label={({ name, value }: { name: string; value: number }) => `${name}: $${value}B`} labelLine>
                  {PIE_IMPORTS_2024.map((_entry, i) => <Cell key={i} fill={P[i % P.length]} />)}
                </Pie>
                <Tooltip {...TT} />
              </PieChart>
            </ResponsiveContainer>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {followUps.map((q, i) => (
            <button key={i} onClick={() => onFollowUp(q)}
              ...
              onMouseEnter={e => { (e.target as HTMLButtonElement).style.borderColor = "#00AAFF"; (e.target as HTMLButtonElement).style.color = "#00AAFF"; }}
              onMouseLeave={e => { (e.target as HTMLButtonElement).style.borderColor = "#2d3348"; (e.target as HTMLButtonElement).style.color = "#94a3b8"; }}>
              {q}
            </button>
          ))}
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// SEARCH MODE  (with Trie-powered autocomplete)
// ─────────────────────────────────────────────────────────────────────────────
function SearchMode() {
  ...
  const trie = getSearchTrie(); // singleton — O(1) after first call

  const handleQueryChange = (val: string) => {
    setQuery(val);
    if (val.trim().length >= 2) {
      const hits = trie.search(val.trim()); // O(m) Trie traversal
      setSuggestions(hits);
  ...
  return (
    <div style={{ maxWidth:860, margin:"0 auto" }}>
      ...
      <div style={{ position:"relative", marginBottom:18 }}>
        <div style={{ display:"flex", gap:10 }}>
          <input
            ...
            style={{ flex:1, background:"#1e2130", border:"1px solid #2d3348", borderRadius:10, padding:"12px 16px", color:"#e2e8f0", fontSize:14, outline:"none", transition:"border-color .15s" }}
            onFocus={e => { e.target.style.borderColor = "#10B981"; }}
          />
          <button
            onClick={() => doSearch(query)}
            disabled={loading || !query.trim()}
            style={{ background:loading || !query.trim() ? "#1e2130" : "#10B981", border:"none", borderRadius:10, padding:"12px 22px", color:loading || !query.trim() ? "#334155" : "#fff", fontSize:14, fontWeight:700, cursor:loading || !query.trim() ? "not-allowed" : "pointer", transition:"all .15s", whiteSpace:"nowrap" }}>
            {loading ? "⏳" : "🔍 Search"}
          </button>
        </div>
          ...
          <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, right:54, background:"#1e2130", border:"1px solid #10B98155", borderRadius:10, zIndex:100, overflow:"hidden" }}>
            ...
            {suggestions.map((s, i) => (
              <button key={i} onMouseDown={() => doSearch(s)}
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = "#10B98118"; (e.currentTarget as HTMLButtonElement).style.color = "#e2e8f0"; }}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:24 }}>
            {SEARCH_SUGGESTIONS.map((s, i) => (
              <button key={i} onClick={() => doSearch(s)}
                style={{ background:"#1e2130", border:"1px solid #2d3348", borderRadius:8, padding:"10px 14px", fontSize:12, color:"#94a3b8", cursor:"pointer", textAlign:"left", lineHeight:1.4, transition:"all .15s" }}
      ...
      {/* Loading */}
      {loading && (
        <div style={{ background:"#1e2130", border:"1px solid #10B98144", borderRadius:12, padding:"22px 24px", display:"flex", alignItems:"center", gap:16 }}>
          <span style={{ animation:"spin 1.8s linear infinite", display:"inline-block", fontSize:22 }}>🔍</span>
          <div>
            <p style={{ margin:"0 0 4px", fontSize:14, color:"#e2e8f0", fontWeight:600 }}>Searching the web…</p>
            <p style={{ margin:0, fontSize:12, color:"#64748b" }}>Querying World Bank, IMF, Reuters, Bloomberg and authoritative sources</p>
          </div>
        </div>
      )}
      ...
      {result && (
        <div>
          ...
          <div style={{ background:"#1e2130", border:`1px solid ${result.webSearchUsed ? "#10B98133" : "#F59E0B33"}`, borderRadius:12, padding:22, marginBottom:14 }}>
            <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:14 }}>
              <span style={{ fontSize:16 }}>🌐</span>
              <span style={{ fontSize:11, fontWeight:700, textTransform:"uppercase", color:result.webSearchUsed ? "#10B981" : "#F59E0B" }}>Research Summary</span>
              {!result.webSearchUsed && (
                <span style={{ fontSize:10, color:"#F59E0B", background:"#F59E0B11", border:"1px solid #F59E0B33", borderRadius:4, padding:"1px 7px" }}>
          ...
          <div style={{ background:"#1e2130", border:"1px solid #2d3348", borderRadius:10, padding:14 }}>
            <p style={{ margin:"0 0 10px", fontSize:11, color:"#64748b", fontWeight:600 }}>Search again or refine:</p>
            <div style={{ display:"flex", gap:10 }}>
              <input
                value={followQuery}
                ...
                style={{ flex:1, background:"#0f1117", border:"1px solid #2d3348", borderRadius:8, padding:"9px 14px", color:"#e2e8f0", fontSize:13, outline:"none" }}
                onFocus={e => { e.target.style.borderColor = "#10B981"; }}
                onBlur={e => { e.target.style.borderColor = "#2d3348"; }}
              />
              <button
...
const MODE_META: Record<Mode, ModeMeta> = {
  dashboard: { label:"Dashboard Mode",   desc:"Pre-built charts with filterable static data — great for overview and reference",                                   color:"#00AAFF" },
  chat:      { label:"AI Chat Mode",     desc:"Prompt-driven · Claude generates charts and analysis dynamically from your query",                                  color:"#8B5CF6" },
  search:    { label:"Web Search Mode",  desc:"Live web search · Claude pulls and summarizes current data from reliable sources",                                  color:"#10B981" },
  data:      { label:"Data Upload Mode", desc:"Upload a CSV file · Claude analyzes your data and creates charts automatically",                                    color:"#F59E0B" },
  analytics: { label:"Analytics Mode",   desc:"Algorithms from scratch: OLS Regression · HHI Concentration · K-Means Clustering · Z-Score Anomaly Detection",    color:"#EF4444" },
};
